...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname JWADDELSIFINAL) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
(define BULLET-MAX-SPACESHIP 6)
(define INVADER-LENGTH 25)
(define INVADER-RECT (bitmap "INVADER.png"))
(define GAP-BETWEEN-ROWS 10)
(define WIDTH (* 2 COL-NUM-INVADER INVADER-LENGTH))
(define HEIGHT 600)
(define BACKGROUND (overlay (rectangle WIDTH HEIGHT "solid" "magenta")
                     (empty-scene WIDTH HEIGHT)))
(define SPACESHIP-L 30)
(define SPACESHIP-W 20)
(define SPACESHIP-RECT (bitmap "SHIP.png"))
...
(define MSHIP-RECT (bitmap "MOTHERSHIP.png"))
...
(define (first-x cols)
  (/ (- WIDTH (* (- cols ONE) TWO INVADER-LENGTH)) TWO))
...
(define BOTTOM-RIGHT-CORNER (make-posn (- WIDTH  30)
                                       (- HEIGHT 30)))
...
(define (mothership-out-of-bounds? mothership)
  (>= (+ (posn-x (spaceship-position mothership))
         (/ INVADER-LENGTH TWO)) WIDTH))

;;;; Tests
(define MSHIP-IN (make-spaceship 
                  (make-posn
                   (- WIDTH (/ INVADER-LENGTH 2) MSHIP-MOVE-LEN) 20) 
                   'right MAX-LIVES-MSHIP))
(define MSHIP-OUT (make-spaceship 
                   (make-posn
                    (- WIDTH (/ INVADER-LENGTH 2)) 20) 
                   'right MAX-LIVES-MSHIP))
...
(define POSN-BOTTOM-MID (make-posn (/ WIDTH 2) (- HEIGHT 20)))
